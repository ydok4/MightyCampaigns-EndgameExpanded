...
local ci_late_game_turn_timeout = 140;
...
local ci_max_num_hordes = {{6, 6, 8, 9, 10}, {10, 10, 12, 13, 14}};

local army_archetypes = {
	Stage1Armies = {
		ArmyArchetypes = {
			chaos = {
				ChaosLord = {
				...
				ChaosSorcerorLord = {
					Key = "ChaosSorcerorLordInvasionStage2",
					Weighting = 2,
				},
...
local ci_spawn_areas = {
	wh_main_chs_chaos = {
		...
		MaxY = 625,
		Stage1Armies = {
			NumberOfArmies = {4, 4, 6, 6, 7},
		},
		Stage2Armies = {
			NumberOfArmies = {7, 7, 8, 9, 10},
		},
	},
	wh_dlc03_bst_beastmen_chaos = {
		Key = "wh_dlc03_bst_beastmen_chaos",
		Army = "beastmen",
		MinX = 766,
		MaxX = 780,
		MinY = 612,
		MaxY = 625,
		Stage1Armies = {
			NumberOfArmies = {3, 3, 3, 4, 4},
		},
		Stage2Armies = {
			NumberOfArmies = {4, 4, 5, 5, 6},
		},
	},
	wh2_main_chs_chaos_incursion_def = {
		Key = "wh2_main_chs_chaos_incursion_def",
		Army = "chaos",
		MinX = 216,
		MaxX = 237,
		MinY = 495,
		MaxY = 507,
		Stage1Armies = {
			NumberOfArmies = {5, 5, 7, 8, 9},
		},
		Stage2Armies = {
			NumberOfArmies = {6, 6, 8, 9, 10},
		},
	},
	wh2_main_chs_chaos_incursion_hef = {
		Key = "wh2_main_chs_chaos_incursion_hef",
		Army = "chaos",
		MinX = 282,
		MaxX = 307,
		MinY = 15,
	...
	--[[wh2_main_skv_clan_pestilens = {
		Key = "wh2_main_skv_clan_pestilens",
		Army = "skaven",
		MinX = 282,
		MaxX = 307,
		MinY = 15,
...
-- easy, normal, hard, very hard, legendary
local ci_num_spawn_waves_late_game = {0, 2, 4, 5, 6};
...
local ci_chaos_horde_buildings = {
	...
	"wh_main_horde_chaos_warriors_4",
	...
	if is_table(chaos_rises_start_pos) and is_string(chaos_rises_scene) then
		...
		cutscene:action(
			function()
				...
				cutscene:cindy_playback(chaos_rises_scene, 0, 5);
			end, 
			0.6
		);
	...
	if cm:get_local_faction(true) then
		...
		core:add_listener(
			...
			function()
				...
				local respawnLimit = ci_spawn_areas["wh_main_chs_chaos"].Stage1Armies[difficulty] / 2;
				if num_hordes < respawnLimit then
...
function ci_late_game_start()
	...
	if cm:is_multiplayer() then
		...
		--MC: There have been some changes here to offset the spawns from each other.
		-- Mods which listen to character spawning can overwhelming the scripting engine
		-- Causing things to break.
		ci_spawn_archaon();
		-- Spawn Sigvald and Kholek
...
function ci_late_game_setup_listeners()
	...
	if cm:get_local_faction(true) then
		core:add_listener(
			...
			function()
				local difficulty = ci_get_difficulty();
				-- This is limited to half the max late game spawn size of the main chaos force
				local respawnLimit = ci_spawn_areas["wh_main_chs_chaos"].Stage2Armies[difficulty] / 2;
				if ci_get_num_hordes() < respawnLimit then
...
function ci_spawn_horde(stage, callback, instant)
	...
	for key, spawnData in pairs(ci_spawn_areas) do
		local faction_name = spawnData.Key;
		if (stage == 1 and spawnData.Stage1Armies ~= nil)
		or (stage == 2 and spawnData.Stage2Armies ~= nil) then
			local current_num_hordes = ci_get_num_hordes(faction_name);
			local max_num_hordes = spawnData.Stage2Armies.NumberOfArmies[difficulty];
			...
			if current_num_hordes >= max_num_hordes then
				...
				for j = 1, desired_hordes do
					...
					local x = 0;
					local y = 0;
					local army_data = false;
					local buildings = "";
					local faction_name_to_spawn = faction_name;

					army_data = ci_get_army_data(faction_name_to_spawn, spawnData.Army, stage);
					out.chaos("army_string is: "..army_data.ArmyString);
					if spawnData.Army == "chaos" then
						buildings = ci_chaos_horde_buildings;
					elseif spawnData.Army == "beastmen" then
						buildings = ci_beastmen_horde_buildings;
					end
					
					if not army_data.ArmyString then
						break;
					end;
					out.chaos("Pre spawn_function");
					local spawn_function = function()
						out.chaos("army spawn_function");
						while not valid do
							x = cm:random_number(spawnData.MaxX, spawnData.MinX);
							y = cm:random_number(spawnData.MaxY, spawnData.MinY);
							
							if is_valid_spawn_point(faction_name_to_spawn, x, y) then
								out.chaos("Got valid spawn point");
								if army_data.AgentData == nil then
									out.chaos("Creating force");
									cm:create_force(
										faction_name_to_spawn,
										army_data.ArmyString,
										...
										function(cqi)
											...
											cm:add_agent_experience(cm:char_lookup_str(cqi), ci_character_xp[stage + 1][cm:random_number(3)], true);
									...
									cm:create_force_with_general(
										...
										y,
										"general",
										army_data.AgentData.AgentSubTypeKey,
										army_data.AgentData.ClanNameKey,
										"",
										army_data.AgentData.ForeNameKey,
										"",
										false,
										function(cqi)
											out.chaos("create_force() has created a [" .. faction_name_to_spawn .. "] force, commander's cqi is " .. cqi);
					...
					if j == 1 or not cm:get_local_faction(true) or instant then
						...
						out.chaos("Getting spawn timer");
						local timer = j - 1;
						
						if is_multiplayer then
							timer = timer / 5;
						end;
						out.chaos("Spawn timer is: "..timer);
						cm:callback(
							function()
								spawn_function();
							end,
...
function ci_setup_armies()
	...
	ram:add_mandatory_unit("chaos_1", "wh_main_chs_inf_chaos_warriors_1",		1);
	ram:add_mandatory_unit("chaos_1", "wh_main_chs_inf_chaos_warriors_0",		1);
	ram:add_mandatory_unit("chaos_1", "wh_dlc01_chs_inf_chaos_warriors_2",		1);
	
	ram:add_unit("chaos_1", "wh_main_chs_inf_chaos_marauders_0",		10);
	ram:add_unit("chaos_1", "wh_main_chs_inf_chaos_marauders_1",		10);
	ram:add_unit("chaos_1", "wh_main_chs_cav_chaos_chariot",			5);
	ram:add_unit("chaos_1", "wh_main_nor_cav_marauder_horsemen_0",		8);
	ram:add_unit("chaos_1", "wh_dlc06_chs_cav_marauder_horsemasters_0",	3);
	ram:add_unit("chaos_1", "wh_main_chs_mon_chaos_warhounds_0",		3);
	ram:add_unit("chaos_1", "wh_main_chs_mon_chaos_warhounds_1",		2);
	ram:add_unit("chaos_1", "wh_main_chs_mon_chaos_spawn",				6);
	ram:add_unit("chaos_1", "wh_main_chs_mon_trolls",					3);
	
	ram:new_force("chaos_2");
	
	ram:add_mandatory_unit("chaos_2", "wh_main_chs_art_hellcannon",				1);
	ram:add_mandatory_unit("chaos_2", "wh_dlc06_chs_inf_aspiring_champions_0",	2);
	ram:add_mandatory_unit("chaos_2", "wh_dlc01_chs_inf_chosen_2",				2);
	ram:add_mandatory_unit("chaos_2", "wh_main_chs_inf_chosen_1",				2);
	ram:add_mandatory_unit("chaos_2", "wh_main_chs_inf_chosen_0",				5);
	ram:add_mandatory_unit("chaos_2", "wh_main_chs_cav_chaos_knights_0",		5);
	ram:add_mandatory_unit("chaos_2", "wh_main_chs_cav_chaos_knights_1",		3);
	
	ram:add_unit("chaos_2", "wh_main_chs_inf_chaos_warriors_0",			10);
	ram:add_unit("chaos_2", "wh_main_chs_inf_chaos_warriors_1",			10);
	ram:add_unit("chaos_2", "wh_dlc01_chs_inf_chaos_warriors_2",		10);
	ram:add_unit("chaos_2", "wh_dlc01_chs_inf_forsaken_0",				8);
	ram:add_unit("chaos_2", "wh_dlc01_chs_mon_trolls_1",				5);
	ram:add_unit("chaos_2", "wh_dlc01_chs_mon_dragon_ogre_shaggoth",	1);
	ram:add_unit("chaos_2", "wh_dlc01_chs_mon_dragon_ogre",				2);
	
	ram:new_force("beastmen_1");
	
	ram:add_mandatory_unit("beastmen_1", "wh_dlc03_bst_inf_chaos_warhounds_1",	1);
	ram:add_mandatory_unit("beastmen_1", "wh_dlc03_bst_feral_manticore",		1);

	--ram:add_unit("beastmen_1", "wh_dlc03_bst_inf_ungor_herd_1",			5);
	ram:add_unit("beastmen_1", "wh_dlc03_bst_inf_ungor_raiders_0",		5);
	--ram:add_unit("beastmen_1", "wh_dlc03_bst_inf_ungor_spearmen_0",		10);
	ram:add_unit("beastmen_1", "wh_dlc03_bst_inf_chaos_warhounds_0",	3);
	ram:add_unit("beastmen_1", "wh_dlc03_bst_inf_gor_herd_0",			10);
	ram:add_unit("beastmen_1", "wh_dlc03_bst_inf_gor_herd_1",			5);
	...
	ram:add_mandatory_unit("beastmen_2", "wh_dlc03_bst_mon_giant_0",	1);
	
	ram:add_unit("beastmen_2", "wh_dlc03_bst_inf_bestigor_herd_0",	10);
	ram:add_unit("beastmen_2", "wh_dlc03_bst_inf_centigors_2",		5);
	ram:add_unit("beastmen_2", "wh_dlc03_bst_inf_minotaurs_0",		8);
	ram:add_unit("beastmen_2", "wh_dlc03_bst_inf_minotaurs_1",		2);
	ram:add_unit("beastmen_2", "wh_dlc03_bst_inf_razorgor_herd_0",	2);
	ram:add_unit("beastmen_2", "wh_dlc03_bst_feral_manticore",		3);
end;

function ci_get_army_data(armyFactionKey, race, stage)
	if core:is_mod_loaded("zz_enhanced_rebellions") then
		...
		if race == "beastmen" then
			armySubculture = "wh_dlc03_sc_bst_beastmen";
		elseif race == "chaos" then
			armySubculture = "wh_main_sc_chs_chaos";
		elseif race == "norsca" then
			armySubculture = "wh_main_sc_nor_norsca";
		elseif race == "skaven" then
			armySubculture = "wh2_main_sc_skv_skaven";
		end
		out.chaos("race: "..race.." armySubculture: "..armySubculture);

		local archetypeKeysForRaceAndStage = {};
		if stage == 1 then 
			archetypeKeysForRaceAndStage = army_archetypes.Stage1Armies.ArmyArchetypes[race];
			-- Norsca doesn't spawn in wave 1 (at this point)
			if archetypeKeysForRaceAndStage == nil then
				return nil;
			end
		elseif stage == 2 then
			archetypeKeysForRaceAndStage = army_archetypes.Stage2Armies.ArmyArchetypes[race];
		end
		local armyArchetypeKey = "";
		local armyArchetypeResources = nil;
		while armyArchetypeResources == nil do
			armyArchetypeKey = GetRandomItemFromWeightedList(archetypeKeysForRaceAndStage, true);
			armyArchetypeResources = _G.ER:GetResourcesForRebelArmyArchetypes(armySubculture, armyArchetypeKey);
		end
        ...
        local encounterArmySize = cm:random_number(ci_horde_size[stage][2], ci_horde_size[stage][1]);
        local ramData = {
            ForceKey = encounterData.ForceKey,
            ArmySize = encounterArmySize,
            ForceData = encounterData,
		};
		local armyString = _G.ER.ArmyGenerator:GenerateForceForTurn(ramData, encounterArmySize);
		out.chaos("Got army string");
		local generatedName = {
			clan_name = "",
			forename = "",
		};
		generatedName = _G.ER.CharacterGenerator:GetCharacterNameForSubculture(armyFaction, agentSubTypeKey);
		agentSubTypeData = {
			AgentSubTypeKey = agentSubTypeKey,
		...
		out.chaos("Using normal generation");
		local armyString = random_army_manager:generate_force(race .. "_" .. stage, cm:random_number(ci_horde_size[stage][2], ci_horde_size[stage][1]));
        return {
			ArmyString = armyString,
			AgentData = nil,
		};
...
function is_valid_spawn_point(faction_name, x, y)
	local spawn_x, spawn_y = cm:find_valid_spawn_location_for_character_from_position(faction_name, x, y, true, 0);
	if x == spawn_x
	and y == spawn_y then
		return true;
	end
...
function ci_spawn_agent(num_agents)
	...
	for i = 1, num_agents do
		...
		while not valid and count < 10 do
			...
			if is_valid_spawn_point(ci_chaos_faction_string, chosen_coord[1], chosen_coord[2]) then
				cm:create_agent(
					...
					true,
					function(cqi)
						cm:add_agent_experience(cm:char_lookup_str(cqi), ci_character_xp[1][cm:random_number(3)], true);
					end
				);
...
function ci_spawn_norsca()
	local ci_norsca_spawn_area = ci_spawn_areas["ci_norsca_spawn_area"];
	local difficulty = ci_get_difficulty();
	...
	local stage = cm:get_saved_value("ci_stage");
	local army_string = {
		ci_get_army_string("norsca", stage),
		ci_get_army_string("norsca", stage),
		ci_get_army_string("norsca", stage),
	};
	...
	for i = 1, num_armies_to_spawn do
		...
		while not valid do
			x = cm:random_number(ci_norsca_spawn_area.MaxX, ci_norsca_spawn_area.MinX);
			y = cm:random_number(ci_norsca_spawn_area.MaxY, ci_norsca_spawn_area.MinY);
			
			if is_valid_spawn_point(norsca_faction_name, x, y) then
				cm:create_force(
					norsca_faction_name,
...
function ci_spawn_special_character(char_details)
	local unit_str = random_army_manager:generate_force("NCI_sarthorael", 19, false);
	local valid = false;
	local x = 0;
	local y = 0;
	local spawnData = ci_spawn_areas["wh_main_chs_chaos"];
	while not valid do
		x = cm:random_number(spawnData.MaxX, spawnData.MinX);
		y = cm:random_number(spawnData.MaxY, spawnData.MinY);
		
		if is_valid_spawn_point(ci_chaos_faction_string, x, y) then
			cm:create_force_with_general(
				ci_chaos_faction_string,
				unit_str,
				ci_chaos_home_region,
...
function ci_spawn_archaon()
	...
	elseif archaon:has_military_force() then
		...
		out.chaos("ci_spawn_archaon");
		local unit_str = random_army_manager:generate_force("NCI_archaon", 19, false);
		local valid = false;
		local x = 0;
		local y = 0;
		local spawnData = ci_spawn_areas["wh_main_chs_chaos"];
...
-- Addition
function ci_spawn_sigvald()
	if not NCI_sigvald then
		script_error("WARNING: Could not find Sigvald to spawn him - how could this happen?");
	else
		local unit_str = random_army_manager:generate_force("NCI_sigvald", 19, false);
		local valid = false;
		local x = 0;
		local y = 0;
		local spawnData = ci_spawn_areas["wh_main_chs_chaos"];
		while not valid do
			...
			if is_valid_spawn_point(ci_chaos_faction_string, x, y) then
				cm:create_force_with_general(
					...
					y,
					"general",
					NCI_sigvald["agent_subtype"],
					NCI_sigvald["forename"],
					"",
					NCI_sigvald["family_name"],
					"",
					NCI_sigvald["faction_leader"],
					function(cqi)
						local char_str = cm:char_lookup_str(cqi);
...
function ci_spawn_kholek()
	if not NCI_kholek then
		script_error("WARNING: Could not find Kholek to spawn him - how could this happen?");
	else
		local unit_str = random_army_manager:generate_force("NCI_kholek", 19, false);
		local valid = false;
		local x = 0;
		local y = 0;
		local spawnData = ci_spawn_areas["wh_main_chs_chaos"];
		while not valid do
			...
			if is_valid_spawn_point(ci_chaos_faction_string, x, y) then
				...
				cm:create_force_with_general(
					...
					y,
					"general",
					NCI_kholek["agent_subtype"],
					NCI_kholek["forename"],
					"",
					NCI_kholek["family_name"],
					"",
					NCI_kholek["faction_leader"],
					function(cqi)
						local char_str = cm:char_lookup_str(cqi);
...
function ci_make_chaos_vassals()
	...
	if stage == 1 then
		vassal_factions = {
			[ci_beastmen_faction_string] = ci_chaos_faction_string,
			["wh_main_nor_varg"] = ci_chaos_faction_string,
			["wh_main_nor_skaeling"] = ci_chaos_faction_string,
			-- Additions
			["wh_dlc08_nor_goromadny_tribe"] = ci_chaos_faction_string,
		};
	-- Late game invasion
	elseif stage == 2 then
		vassal_factions = {
			...
			-- Additions
			["wh_dlc08_nor_goromadny_tribe"] = ci_chaos_faction_string,
			["wh_main_nor_sarl"] = ci_chaos_faction_string,
			["wh_main_nor_graeling"] = ci_chaos_faction_string,
			["wh_main_nor_baersonling"] = ci_chaos_faction_string,
			["wh_main_nor_aesling"] = ci_chaos_faction_string,
			["wh_dlc08_nor_vanaheimlings"] = ci_chaos_faction_string,
			["wh_dlc08_nor_naglfarlings"] = ci_chaos_faction_string,
			["wh_dlc08_nor_helspire_tribe"] = ci_chaos_faction_string,
			-- Sourth west
			["wh_main_nor_bjornling"] = "wh2_main_chs_chaos_incursion_hef",
			-- North west
			["wh2_main_nor_mung"] = "wh2_main_chs_chaos_incursion_def",
			["wh2_main_nor_aghol"] = "wh2_main_chs_chaos_incursion_def",
			-- We don't vassalise these 2 but we do disable war
			["wh_dlc08_nor_wintertooth"] = ci_chaos_faction_string,
			["wh_dlc08_nor_norsca"] = ci_chaos_faction_string,
		};
	end
	...
	for current_faction, master_faction in pairs(vassal_factions) do
		if make_norsca_vassals or current_faction == ci_beastmen_faction_string then
			out.chaos("force_make_vassal between " .. master_faction .. " and " .. current_faction);
			if current_faction ~= "wh_dlc08_nor_wintertooth"
			and current_faction ~= "wh_dlc08_nor_norsca" then
				cm:force_make_vassal(master_faction, current_faction);
			end
		end;
	end;
	...
	cm:callback(function()
		for current_faction, master_faction in pairs(vassal_factions) do
			if make_norsca_vassals or current_faction == ci_beastmen_faction_string then
				out.chaos("Setup vassal diplomacy options between " .. ci_chaos_faction_string .. " and " .. current_faction);
				cm:force_diplomacy("faction:"..ci_chaos_faction_string..",wh2_main_chs_chaos_incursion_def,wh2_main_chs_chaos_incursion_hef", "faction:" .. current_faction, "war,break vassal,break alliance,break client state", false, false, true);
			end
		end
	end,
	1);
...
function ci_get_num_hordes(faction_name)
	if faction_name == "wh_main_chs_chaos" or not faction_name then
